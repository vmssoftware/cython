...
// see http://bugs.python.org/issue21420
    ...
    if (likely(shiftby >= 0)) {
        if ((size_t)shiftby <= sizeof(int) * 8 - 2) {
            int value = 1L << shiftby;
...
static CYTHON_INLINE {{c_ret_type}} __Pyx_PyInt_{{'' if ret_type.is_pyobject else 'Bool'}}{{op}}{{order}}(PyObject *op1, PyObject *op2, int intval, int inplace); /*proto*/
...
static CYTHON_INLINE {{c_ret_type}} __Pyx_PyInt_{{'' if ret_type.is_pyobject else 'Bool'}}{{op}}{{order}}(PyObject *op1, PyObject *op2, CYTHON_UNUSED int intval, CYTHON_UNUSED int inplace) {
    if (op1 == op2) {
    ...
    if (likely(PyInt_CheckExact({{pyval}}))) {
        const int {{'a' if order == 'CObj' else 'b'}} = intval;
        int {{ival}} = PyInt_AS_LONG({{pyval}});
    ...
    if (likely(PyLong_CheckExact({{pyval}}))) {
        ...
        if (uintval >> (PyLong_SHIFT * {{_size}})) {
            ...
            unequal = (size != {{_size+1}}) || (digits[0] != (uintval & (unsigned int) PyLong_MASK))
                {{for _i in range(1, _size+1)}} | (digits[{{_i}}] != ((uintval >> ({{_i}} * PyLong_SHIFT)) & (unsigned int) PyLong_MASK)){{endfor}};
        } else
#endif
        {{endfor}}
            unequal = (size != 1) || (((unsigned int) digits[0]) != (uintval & (unsigned int) PyLong_MASK));
...
{{py: c_ret_type = 'PyObject*' if ret_type.is_pyobject else 'int'}}
#if !CYTHON_COMPILING_IN_PYPY
static {{c_ret_type}} __Pyx_PyInt_{{'' if ret_type.is_pyobject else 'Bool'}}{{op}}{{order}}(PyObject *op1, PyObject *op2, int intval, int inplace, int zerodivision_check); /*proto*/
#else
...
static {{c_ret_type}} {{cfunc_name}}(PyObject *op1, PyObject *op2, CYTHON_UNUSED int intval, int inplace, int zerodivision_check) {
    ...
    if (likely(PyInt_CheckExact({{pyval}}))) {
        ...
        {{elif op == 'TrueDivide'}}
            {{zerodiv_check('b')}}
            if (8 * sizeof(int) <= 53 || likely(labs({{ival}}) <= ((PY_LONG_LONG)1 << 53))) {
        ...
        {{elif op == 'FloorDivide'}}
            ...
            {{zerodiv_check('b')}}
            if (unlikely(b == -1 && ((unsigned int)a) == 0-(unsigned int)a))
                return PyInt_Type.tp_as_number->nb_{{slot_name}}(op1, op2);
            else {
                int q, r;
            ...
            return PyInt_FromLong(x);
        {{elif op == 'Lshift'}}
            if (likely(b < (int) (sizeof(int)*8) && a == (a << b) >> b) || !a) {
    ...
    if (likely(PyLong_CheckExact({{pyval}}))) {
        const int {{'a' if order == 'CObj' else 'b'}} = intval;
        int {{ival}}{{if op not in ('Eq', 'Ne')}}, x{{endif}};
        ...
        if (likely(__Pyx_sst_abs(size) <= 1)) {
            ...
            switch (size) {
                ...
                case {{_case}}:
                    if (8 * sizeof(int) - 1 > {{_size}} * PyLong_SHIFT{{if op == 'TrueDivide'}} && {{_size-1}} * PyLong_SHIFT < 53{{endif}}) {
                        {{ival}} = {{'-' if _case < 0 else ''}}(int) {{pylong_join(_size, 'digits')}};
                        break;
                    {{if op not in ('Eq', 'Ne', 'TrueDivide')}}
#ifdef HAVE_LONG_LONG
        ...
        {{else}}
            ...
            {{else}}
                ...
                {{if op == 'Lshift'}}
#ifdef HAVE_LONG_LONG
                if (unlikely(!(b < (int) (sizeof(int)*8) && a == x >> b)) && a) {
                    ll{{ival}} = {{ival}};
                    goto long_long;
                }
#else
                if (likely(b < (int) (sizeof(int)*8) && a == x >> b) || !a) /* execute return statement below */
#endif
                {{endif}}
            {{endif}}
...
static {{c_ret_type}} {{cfunc_name}}(PyObject *op1, PyObject *op2, double floatval, int inplace, int zerodivision_check) {
    ...
    if (likely(PyLong_CheckExact({{pyval}}))) {
        ...
        switch (size) {
            ...
            case -{{_size}}:
            case {{_size}}:
                if (8 * sizeof(unsigned int) > {{_size}} * PyLong_SHIFT && ((8 * sizeof(unsigned int) < 53) || ({{_size-1}} * PyLong_SHIFT < 53))) {
                    ...
                    if ((8 * sizeof(unsigned int) < 53) || ({{_size}} * PyLong_SHIFT < 53) || ({{fval}} < (double) ((PY_LONG_LONG)1 << 53))) {
                        if (size == {{-_size}})
                            {{fval}} = -{{fval}};
                        break;
                    }
