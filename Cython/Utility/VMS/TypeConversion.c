...
#define __Pyx_long_cast(x) ((int)x)
...
static CYTHON_INLINE PyObject * __Pyx_PyBool_FromLong(int b);
...
static Py_UCS4 __Pyx__PyObject_AsPy_UCS4_raise_error(int ival) {
   if (ival < 0) {
       if (!PyErr_Occurred())
...
#pragma GCC diagnostic pop
    ...
    if (likely(PyInt_Check(x))) {
        if (sizeof({{TYPE}}) < sizeof(int)) {
            __PYX_VERIFY_RETURN_INT({{TYPE}}, int, PyInt_AS_LONG(x))
        } else {
            int val = PyInt_AS_LONG(x);
    ...
    if (likely(PyLong_Check(x))) {
        if (is_unsigned) {
            ...
            switch (Py_SIZE(x)) {
                ...
                case {{_size}}:
                    if (8 * sizeof({{TYPE}}) > {{_size-1}} * PyLong_SHIFT) {
                        if (8 * sizeof(unsigned int) > {{_size}} * PyLong_SHIFT) {
                            __PYX_VERIFY_RETURN_INT({{TYPE}}, unsigned int, {{pylong_join(_size, 'digits')}})
            ...
            if (sizeof({{TYPE}}) <= sizeof(unsigned int)) {
                __PYX_VERIFY_RETURN_INT_EXC({{TYPE}}, unsigned int, PyLong_AsUnsignedLong(x))
#ifdef HAVE_LONG_LONG
            } else if (sizeof({{TYPE}}) <= sizeof(unsigned PY_LONG_LONG)) {
...
#endif
            if (sizeof({{TYPE}}) <= sizeof(int)) {
                __PYX_VERIFY_RETURN_INT_EXC({{TYPE}}, int, PyLong_AsLong(x))
#ifdef HAVE_LONG_LONG
            } else if (sizeof({{TYPE}}) <= sizeof(PY_LONG_LONG)) {
